...
1.  Open Visual studio code. You will be asked to configure it.
    ...
    1. **Windows users only**: we'll occasionally be using `bash` in the terminal mode of VSCode. Mac and Linux users have `bash` natively installed in the operating system. Windows users will have gotten a copy of bash when they downloaded and installed Git SCM. However, you need to tell VSCode to use `bash` in the Terminal rather than Windows PowerShell. To do that:
        a.  Click "Unlock productivity with the Command Palette" in the Walthrough tab, then "Open Command Palette". Alternatively, if you don't see that option, open the Command Palette by typing <Ctrl> + <Shift> + <P>.
...
1.  Open a terminal by click "Terminal" -> "New Terminal".
...
1.  Type:
    ...
    ```
1.  After it has finished installing, open the Command Palette by typing <Ctrl> + <Shift> + <P>. (Mac users: type <command> instead of <Ctrl>.) 
...
1.  Check to ensure your environment works:
    ...
    1.  In the first cell, type:
        ```python
        print("hello world!")
    1.  Hit the run button or type <Shift> + <Enter> to execute the code. You should get back "hello world!".
